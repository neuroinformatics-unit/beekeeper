...
def test_columns_names_and_nrows_in_df_from_metadata(
    ...
    if diff == {"ROIs"}:
        diff = set()
    assert not diff, (
        "Metadata fields and df columns differ in the following fields: "
        f"{diff}"
    )
...
def test_df_from_metadata_yaml_no_metadata(metadata_fields) -> None:
    """Test with no metadata files (expect just to create an empty
    dataframe with metadata_fields column headers).
    """
...
def test_create_metadata_table_component_column_ordering(
    ...
    config = {
        "metadata_key_field_str": key_field,
        "videos_dir_path": "/fake/path",
    }
    ...
    assert first_column_id == key_field, (
    ...
    assert not first_column_hideable, (
